...
export type XlsxReadMetrics = { unzipMs: number; xmlMs: number; stylesMs: number; sheetModelMs: number; totalMs: number }
...
export async function readXlsx(bytes: Uint8Array, onMetrics?: (metrics: XlsxReadMetrics) => void): Promise<XlsxBook> {
  const started=performance.now();let xmlMs=0,stylesMs=0,sheetModelMs=0
  let zip: JSZip
  try { zip=await JSZip.loadAsync(bytes) } catch { throw new XlsxStructureError('El archivo no es un paquete XLSX válido.') }
  const unzipMs=performance.now()-started
  const read=async(path:string,required=true)=>{const file=zip.file(path);if(!file){if(required)throw new XlsxStructureError(`El XLSX no contiene ${path}.`);return null}const before=performance.now();const document=parseXml(await file.async('text'),path);xmlMs+=performance.now()-before;return document}
  ...
  const stylesStarted=performance.now()
  ...
  stylesMs=performance.now()-stylesStarted
  ...
  for(const sheetNode of Array.from(workbook!.getElementsByTagNameNS('*','sheet'))) {
    ...
    const document=await read(resolvePath('xl/workbook.xml',relation.target)),modelStarted=performance.now(),cells=new Map<string,XlsxCell>();let rows=0,columns=0,lastRow=0
    ...
    sheetModelMs+=performance.now()-modelStarted
  }
  onMetrics?.({unzipMs,xmlMs,stylesMs,sheetModelMs,totalMs:performance.now()-started})
  return {sheets}
}
