...
export const actions: BuilderAction[] = [
  ...
  ...([
    ['ui-showinfo', 'Mostrar información', 'showinfo', false, 'Información', 'Datos guardados correctamente'],
    ['ui-showwarning', 'Mostrar advertencia', 'showwarning', false, 'Advertencia', 'Debe completar todos los campos'],
    ['ui-showerror', 'Mostrar error', 'showerror', false, 'Error', 'No fue posible guardar los datos'],
    ['ui-askstring', 'Pedir texto', 'askstring', true, 'Nombre', 'Ingrese su nombre'],
    ['ui-askinteger', 'Pedir entero', 'askinteger', true, 'Edad', 'Ingrese su edad'],
    ['ui-askfloat', 'Pedir decimal', 'askfloat', true, 'Precio', 'Ingrese el precio'],
    ['ui-askyesno', 'Preguntar Sí / No', 'askyesno', true, 'Confirmación', '¿Desea continuar?'],
    ['ui-askokcancel', 'Preguntar Aceptar / Cancelar', 'askokcancel', true, 'Confirmación', '¿Desea guardar los cambios?'],
  ] as const).map(([id, title, method, returnsValue, defaultTitle, defaultMessage]): BuilderAction => ({
    id,
    title,
    category: 'Presentation / UI · Ventanas emergentes',
    level: 2,
    description: `${title} mediante un diálogo propio de COA GUI.`,
    fields: [
      ...(returnsValue ? [name('variable', 'Variable donde guardar resultado', method === 'askstring' ? 'nombre' : method === 'askinteger' ? 'edad' : method === 'askfloat' ? 'precio' : 'respuesta')] : []),
      text('title', 'Título', defaultTitle),
      text('message', 'Mensaje', defaultMessage),
    ],
    template: `${returnsValue ? 'resultado = ' : ''}gui.${method}("Título", "Mensaje")`,
    generate: (v) => `${returnsValue ? `${v.variable} = ` : ''}gui.${method}(\n    ${quote(v.title)},\n    ${quote(v.message)}\n)`,
  })),
