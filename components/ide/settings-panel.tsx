...
export function SettingsPanel({ settings, profiles, customBackgroundUrl, onChange, onProfiles, onImage, onRemoveImage, onReset, notice }: {
  ...
  return <section className="ide-panel settings-panel">
    ...
    {section === 'background' && <div className="settings-section">
      ...
      <input ref={upload} hidden type="file" accept="image/png,image/jpeg,image/webp,image/gif" onChange={async (e) => { const file=e.target.files?.[0]; if (!file) return; if (!['image/png','image/jpeg','image/webp','image/gif'].includes(file.type) || file.size > 5_000_000) { notice('Usa una imagen PNG, JPG, WEBP o GIF de hasta 5 MB.'); return } await onImage(file); set('background','custom') }}/><button className="ide-secondary" onClick={() => upload.current?.click()}>Subir imagen</button>{customBackgroundUrl && <><div className="background-preview" role="img" aria-label="Vista previa del fondo personalizado" style={{backgroundImage:`url("${customBackgroundUrl}")`}}/><button onClick={async()=>{await onRemoveImage();set('background','none')}}>Quitar imagen</button></>}<small>Tu fondo personalizado, incluido GIF animado, se guarda localmente en este navegador.</small>
