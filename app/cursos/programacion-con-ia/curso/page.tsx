...
export const metadata: Metadata = {
  title: 'Módulo 1: Fundamentos del desarrollo asistido por IA | C.O.A.',
  description: 'Aprende a utilizar asistentes de Inteligencia Artificial sin perder el control del problema, el código, los datos ni las decisiones.',
}
...
const moduleOneDelivery = [{
  match: 'módulo 1',
  fileName: 'COA_IA_M1_Nombre_Apellido.zip',
  items: ['Seis ejercicios obligatorios', 'Comparador de soluciones asistidas', 'Validador de pedidos y descuentos', 'Bitácora, matriz de comparación y evidencias', 'Evaluación práctica resuelta'],
...
function getModuleContent() {
  const source = readFileSync(path.join(process.cwd(), 'content', 'programacion-ia-modulo-1.md'), 'utf8')
...
export default function AiAssistedProgrammingModuleOnePage() {
      ...
      <main className="flex-1">
        <section className="bg-primary text-primary-foreground">
          <div className="mx-auto max-w-5xl px-4 py-10 md:py-14">
            <Link href="/cursos/programacion-con-ia/inscripcion" className="mb-7 inline-flex items-center gap-2 text-sm font-medium text-primary-foreground/80 transition-colors hover:text-primary-foreground"><ArrowLeft className="h-4 w-4" /> Volver a la inscripción</Link>
            <p className="mb-3 font-semibold text-accent">COA — Programación Asistida por Inteligencia Artificial</p>
            <h1 className="max-w-4xl text-balance text-3xl font-extrabold md:text-5xl">Módulo 1: Fundamentos del desarrollo asistido por Inteligencia Artificial</h1>
            <div className="mt-7 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4">
              {[[Clock, 'Duración aproximada', '2 horas'], [GraduationCap, 'Nivel', 'Intermedio'], [BookOpen, 'Requisito', 'Fundamentos de programación'], [Code2, 'Lenguaje', 'El que ya conoces']].map(([Icon, title, value]) => {
            ...
            <div className="mt-4 space-y-1 text-sm text-primary-foreground/80"><p>Herramientas: ChatGPT, Claude, Gemini, GitHub Copilot o equivalentes</p><p>Resultado: Validador de pedidos y descuentos</p></div>
            <div className="mt-5 flex flex-wrap gap-2 text-xs font-semibold">{['6 ejercicios obligatorios', '1 reto opcional', '1 mini proyecto', '1 proyecto', '1 evaluación'].map((item) => <span key={item} className="rounded-full bg-primary-foreground/10 px-3 py-1.5 ring-1 ring-primary-foreground/20">{item}</span>)}</div>
          </div>
        </section>

        <ModuleExperience moduleId="programacion-ia-modulo-1" moduleLabel="Módulo 1" items={toc} nextHref="/cursos/programacion-con-ia/curso/modulo-2" nextLabel="Ir al Módulo 2" completionTitle="¡Has completado el Módulo 1 de Programación Asistida por IA!" completionDescription="Revisa tus actividades, descarga las plantillas y confirma la entrega única antes de avanzar.">
          <article className="space-y-10">
            {sections.map((section) => {
              const heading = section.split('\n', 1)[0]
              const isPractice = /^# (13\.|Ejercicios individuales)/.test(heading)
              const isChallenges = /^# Reto adicional/.test(heading)
            ...
            <div className="mt-16 flex flex-col justify-between gap-3 border-t border-border pt-8 sm:flex-row">
              <Link href="/cursos/programacion-con-ia" className="inline-flex h-9 items-center justify-center gap-2 rounded-lg bg-primary px-4 text-sm font-semibold text-primary-foreground transition-colors hover:bg-primary/90"><ArrowLeft className="h-4 w-4" /> Ver información del curso</Link>
              <Link href="/cursos/programacion-con-ia/curso/modulo-2" className="inline-flex h-9 items-center justify-center gap-2 rounded-lg bg-accent px-4 text-sm font-semibold text-accent-foreground transition-colors hover:bg-accent/90">Módulo 2 <ArrowRight className="h-4 w-4" /></Link>
            </div>
          </article>
        </ModuleExperience>
      </main>
