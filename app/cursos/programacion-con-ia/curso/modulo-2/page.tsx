...
export const metadata: Metadata = {
  title: 'Módulo 2: Prompts y conversaciones técnicas para programar | C.O.A.',
  description: 'Aprende a definir prompts técnicos, preparar contexto y desarrollar software mediante conversaciones estructuradas y verificables.',
}
...
const moduleOneDelivery = [{
  match: 'módulo 2',
  fileName: 'COA_IA_M2_Nombre_Apellido.zip',
  items: ['Ocho ejercicios obligatorios', 'Biblioteca con siete plantillas', 'Cotizador de servicios en sus tres versiones', 'Casos CP-01 a CP-16, evidencias y bitácora', 'Evaluación práctica resuelta'],
...
function getModuleContent() {
  const source = readFileSync(path.join(process.cwd(), 'content', 'programacion-ia-modulo-2.md'), 'utf8')
...
export default function AiAssistedProgrammingModuleTwoPage() {
      ...
      <main className="flex-1">
        <section className="bg-primary text-primary-foreground">
          <div className="mx-auto max-w-5xl px-4 py-10 md:py-14">
            <Link href="/cursos/programacion-con-ia/curso" className="mb-7 inline-flex items-center gap-2 text-sm font-medium text-primary-foreground/80 transition-colors hover:text-primary-foreground"><ArrowLeft className="h-4 w-4" /> Volver al Módulo 1</Link>
            <p className="mb-3 font-semibold text-accent">COA — Programación Asistida por Inteligencia Artificial</p>
            <h1 className="max-w-4xl text-balance text-3xl font-extrabold md:text-5xl">Módulo 2: Prompts y conversaciones técnicas para programar</h1>
            <div className="mt-7 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4">
              {[[Clock, 'Duración aproximada', '3 horas'], [GraduationCap, 'Nivel', 'Intermedio'], [BookOpen, 'Requisito', 'Módulo 1 aprobado'], [Code2, 'Lenguaje', 'El que ya conoces']].map(([Icon, title, value]) => {
            ...
            <div className="mt-4 space-y-1 text-sm text-primary-foreground/80"><p>Herramientas: Asistente conversacional o integrado en el editor</p><p>Resultado: Cotizador de servicios construido mediante conversación estructurada</p></div>
        ...
        <ModuleExperience moduleId="programacion-ia-modulo-2" moduleLabel="Módulo 2" items={toc} nextHref="/cursos/programacion-con-ia/curso/modulo-3" nextLabel="Ir al Módulo 3" completionTitle="¡Has completado el Módulo 2 de Programación Asistida por IA!" completionDescription="Revisa tus actividades, descarga las plantillas y confirma la entrega única antes de avanzar.">
          <article className="space-y-10">
            ...
            <div className="mt-16 flex flex-col justify-between gap-3 border-t border-border pt-8 sm:flex-row">
              <Link href="/cursos/programacion-con-ia/curso" className="inline-flex h-9 items-center justify-center gap-2 rounded-lg bg-primary px-4 text-sm font-semibold text-primary-foreground transition-colors hover:bg-primary/90"><ArrowLeft className="h-4 w-4" /> Módulo 1</Link>
              <Link href="/cursos/programacion-con-ia/curso/modulo-3" className="inline-flex h-9 items-center justify-center gap-2 rounded-lg bg-accent px-4 text-sm font-semibold text-accent-foreground transition-colors hover:bg-accent/90">Módulo 3 <ArrowRight className="h-4 w-4" /></Link>
            </div>
          </article>
        </ModuleExperience>
      </main>
